...
Base64 encoded API key in the format `base64(id:api_key)`. This is an alternative to specifying `api_key_id` and `api_key` separately.

**Note:** You can use either `api_key_id` + `api_key` OR `api_key_encoded`, but not both.
...
source {
    Elasticsearch {
        hosts = ["https://localhost:9200"]
        auth_type = "api_key"
        api_key_encoded = "eW91cl9hcGlfa2V5X2lkOnlvdXJfYXBpX2tleV9zZWNyZXQ="
        index = "my_index"
    }
}
```
...
OAuth2 authentication allows integration with OAuth2/OIDC providers for enterprise authentication scenarios.

#### oauth_client_id [string]

OAuth2 client ID provided by your OAuth2 provider.

#### oauth_client_secret [string]

OAuth2 client secret provided by your OAuth2 provider.

#### oauth_token_url [string]

OAuth2 token endpoint URL where the connector will obtain access tokens.

#### oauth_scope [string]

Optional OAuth2 scope to request. If not specified, no scope will be requested.
...
source {
    Elasticsearch {
        ...
        oauth_scope = "elasticsearch"
        index = "my_index"
    }
}
```

**Note:** OAuth2 authentication requires that your Elasticsearch cluster is configured to accept OAuth2/JWT tokens, typically through SAML or OIDC realm configuration.
